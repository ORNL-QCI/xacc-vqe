...
//     xacc::info("Tr(rhopqrs) during purification: " + std::to_string(tmp_trace));
...
//     filtered_rhopqrs = (1.0 / tmp_val2) * filtered_rhopqrs;
  ...
  for (int p = 0; p < nQubits; p++){
      for (int q = 0; q < nQubits; q++){
          for (int r = 0; r < nQubits; r++){
              for (int s = 0; s < nQubits; s++){
                  rho_pqrs_Sym(filtered_rhopqrs, p, q, r, s) = filtered_rhopqrs(p, q, r, s);
                  rho_pqrs_Sym(filtered_rhopqrs, r, s, p, q) = filtered_rhopqrs(p, q, r, s);
              }
          }
      }
  }
  ...
  for (int p = 0; p < nQubits; p++) {
    for (int q = 0; q < nQubits; q++) {
      for (int r = 0; r < nQubits; r++) {
        for (int s = 0; s < nQubits; s++) {
          energy += 0.5 * std::real(hpqrs(p, q, r, s) * (filtered_rhopqrs(p, q, s, r) +
  ...
  for (int p = 0; p < nQubits; p++){
      for (int q = 0; q < nQubits; q++){
          energy += 0.5 * std::real(hpq(p, q) * (rhopq_tensor(p, q) + rhopq_tensor(q, p)));
      }
  }
